            ...
            <Container style={{ overflow: 'hidden' }}>
                <PublicGamesListItemContainerInner>
                    <AlignLeft>
                        ...
                        {games.map(game => (
                            <p key={game._id}>
                                <PublicGameName>{game.title}</PublicGameName>{' '}
                                <PublicGamePlatform>{game.platform}</PublicGamePlatform>{' '}
                                {title === GameStatus.WISHLIST && game.release_date && (
                                    <PublicGameDate>
                                        {moment(new Date(game.release_date)).format('DD/MM/YYYY')}
                                    </PublicGameDate>
                                )}
                            </p>
                        ))}
