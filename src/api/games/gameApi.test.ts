...
describe("authApi", () => {
  ...
  it("should fetch games for public view", async () => {
    ...
    expect(actualResponse).toEqual(expectedResponse);
    expect(httpClientMock.get).toBeCalledWith(
      `${hostName}/api/v1/public/games/user/${userId}`
    );
  });
  ...
  it("should add game for user", async () => {
    ...
    expect(httpClientMock.post).toBeCalledWith(
      `${hostName}/api/v1/games/user/${userId}`,
      games,
      config
    );
  });
  ...
  it("should update game for user", async () => {
    ...
    expect(actualResponse).toEqual(expectedResponse);
    expect(httpClientMock.post).toBeCalledWith(
      `${hostName}/api/v1/games/${gameId}/user/${userId}`,
      games,
      config
    );
  });
  ...
  it("should delete game for user", async () => {
    ...
    const expectedResponse = mockGamesResponse.data;
    expect(actualResponse).toEqual(expectedResponse);
    expect(httpClientMock.delete).toBeCalledWith(
      `${hostName}/api/v1/games/${gameId}/user/${userId}`,
      config
    );
  });
