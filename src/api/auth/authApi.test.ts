...
describe('authApi', () => {
    ...
    it('should login successfully and return user data', async () => {
        ...
        expect(httpClientMock.post).toBeCalledWith(`${hostName}/api/v1/auth/login`, { password, username });
    });
    ...
    it('should register new user successfully and return user data', async () => {
        ...
        const expectedResponse = mockResponse.data;
        expect(actualResponse).toEqual(expectedResponse);
        expect(httpClientMock.post).toBeCalledWith(`${hostName}/api/v1/auth/register`, { password, username });
    });
});
