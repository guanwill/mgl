...
import { IGame, IGamesApiResponse, IUserGamesStore, IGameToAdd, IGameLocal } from '../../model/game/game';
...
export const callFetchGamesApi = (user_id: string): Function => async (
    ...
    const { games }: IGamesApiResponse = await api.gameApi.fetchGames(user_id, config);
    const sortedGames = games.sort((a, b) => (a.title > b.title ? 1 : b.title > a.title ? -1 : 0));
    return dispatch(loadedGames(sortedGames));
};

export const callAddGameApi = (user_id: string, gameArgs: Partial<IGameLocal>): Function => async (
    dispatch: ThunkDispatch<AppState, void, IGameAction>,
    state: AppState,
    api: IApi
) => {
    ...
    const gamesResponse = await api.gameApi.addGame(user_id, gameArgs, config);
...
export const callUpdateGameApi = (gameArgs: Partial<IGameLocal>, user_id: string, game_id: string): Function => async (
    dispatch: ThunkDispatch<AppState, void, IGameAction>,
    state: AppState,
    api: IApi
) => {
    ...
    return await api.gameApi.updateGame(gameArgs, user_id, game_id, config);
